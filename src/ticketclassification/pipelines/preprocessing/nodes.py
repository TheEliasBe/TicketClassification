...
def filter_first_message_per_ticket(contents: pd.DataFrame) -> pd.DataFrame:
    ...
    contents['timestamp'] = pd.to_datetime(contents['Datum'] + ' ' + contents['Uhrzeit'])
    ...
    initial_messages = initial_messages.merge(contents, on=['ID', 'timestamp'], how='left')
...
def translate_to_english(merged_df: pd.DataFrame):
    ...
    merged_df["Text"] = merged_df["Text"].apply(lambda x: argostranslate.translate.translate(x, from_code, to_code) if langdetect.detect(x) == "de" else x)
    return merged_df
...
def stop_word_removal(tickets: pd.DataFrame):
    ...
    # remove stop words
    stop_words = {'but', 'their', 'our', 'she', 'any', 'has', 'own', 'themselves', 'can', 'what', 'not', 'same', 'been', 'those', 'now', 'ain', 'very', 'why', 'shouldn', 'were', 'then', 'weren', 'myself', 'while', 'about', 'no', 'here', 'above', 'both', "aren't", 'into', 'at', 'don', 'wasn', 'hers', 'off', 'more', 'between', 'its', "didn't", 'shan', 'ours', 'wouldn', "you'd", 'it', 'as', 'each', 'over', 'mightn', 'won', 'was', 'haven', 'yours', 'and', 'up', 'such', 'if', 'with', 'nor', 'in', 'of', 'by', 'i', 'there', 's', 'should', 'when', "should've", 'some', "shan't", 'herself', 'all', "won't", "shouldn't", 'them', 'down', 'further', "that'll", 'o', 'once', "couldn't", 'his', 'your', 'on', "haven't", 'to', 'didn', 'during', "you'll", 'an', 'we', 'most', 'am', 'me', "you've", 'hasn', 'itself', 'himself', 'ourselves', 'whom', 'that', 'll', 'my', 'through', 'y', 'until', "mightn't", 'couldn', 'this', 'they', 'than', 'yourself', 'he', 'before', 'her', "it's", 'from', 'him', 'are', 'which', 'just', 'you', 've', 'against', "doesn't", 'will', 'be', 'doing', "don't", 't', 'theirs', 'too', 'out', 'for', 'being', 'having', 'isn', 'below', 'm', "she's", 'd', "weren't", 'needn', 'have', "you're", 'does', 'other', 'so', 'is', 'where', 'doesn', 'hadn', 'few', "hadn't", "mustn't", 'only', 'these', 'under', 'ma', 'the', 'yourselves', "isn't", 'who', 'how', "hasn't", "wouldn't", 'do', 'because', "needn't", 'after', "wasn't", 're', 'did', 'again', 'aren', 'a', 'mustn', 'had', 'or'}
    tickets["Text"] = tickets["Text"].apply(lambda x: [word for word in x if word not in stop_words])
